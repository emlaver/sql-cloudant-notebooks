...
2. Import the [Node-RED application code](cloudant-to-dashdb.json)
3. Review the Node-RED application code
...
2. `Cloudant in` (Extract all animaldb documents)
    ...
    Note: Since we will be using an existing Cloudant account, the Cloudant service provided by the Node-RED runtime is not required.
